...
A `Topic` contains the query `name` and query `params`. Topic inputs are cloned when the topic is created, so later caller mutation cannot change the query inputs. Structurally equal topics share listeners; `subscribe` returns the listener ID required by `unsubscribe`.
